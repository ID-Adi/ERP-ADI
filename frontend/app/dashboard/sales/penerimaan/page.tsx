...
const dummyReceipts = [
    {
        id: '1',
        receiptNumber: 'RC.2024.12.00050',
    ...
    {
        id: '2',
        receiptNumber: 'RC.2024.12.00051',
...
export default function ReceiptsPage() {
    const [receipts] = useState(dummyReceipts);
    ...
    return (
        <div className="flex flex-col h-[calc(100vh-6rem)] bg-white rounded-lg shadow-sm border border-surface-200 overflow-hidden">
            ...
            <div className="flex items-center gap-2 px-4 py-2 border-b border-surface-200 bg-white overflow-x-auto">
                ...
                <FilterButton label="Payment Method" value="All" />
            ...
            <div className="flex items-center justify-between px-4 py-2 bg-surface-50 border-b border-surface-200">
                <div className="flex items-center gap-2">
                    <Link href="/dashboard/sales/penerimaan/new">
                    ...
                    <tbody className="divide-y divide-surface-200">
                        {filteredReceipts.map((r, index) => (
                            ...
                            >
                                ...
                                <td className="px-4 py-2 font-medium text-warmgray-900">{r.receiptNumber}</td>
                                <td className="px-4 py-2 text-primary-600 font-medium">{r.invoiceNumber}</td>
                                <td className="px-4 py-2 text-warmgray-600 whitespace-nowrap">{formatDate(r.receiptDate)}</td>
                                <td className="px-4 py-2 text-warmgray-900 font-medium">{r.customerName}</td>
                                <td className="px-4 py-2">
                                    <span className="px-2 py-1 rounded-full text-xs font-semibold bg-surface-200 text-warmgray-700">{r.paymentMethod}</span>
                                </td>
                                <td className="px-4 py-2 text-right font-bold text-emerald-600">{formatCurrency(r.amount)}</td>
                            </tr>
                        ))}
            ...
            <div className="px-4 py-2 border-t border-surface-200 bg-surface-50 flex items-center justify-between text-xs text-warmgray-500">
                <span>Showing 1 to {filteredReceipts.length} of {filteredReceipts.length} items</span>

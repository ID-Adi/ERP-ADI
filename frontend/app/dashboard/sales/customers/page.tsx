...
const dummyCustomers = [
    ...
    },
    {
        ...
        name: 'TB. KARYA JASA',
        phone: '0541-123456',
        email: 'karya@jasa.co.id',
        address: 'Jl. Raya Balikpapan, Kaltim',
        category: 'Contractor',
    ...
    return (
        <div className="flex flex-col h-[calc(100vh-6rem)] bg-white rounded-lg shadow-sm border border-surface-200 overflow-hidden">
            ...
            <div className="flex items-center gap-2 px-4 py-2 border-b border-surface-200 bg-white overflow-x-auto">
                <FilterButton label="Category" value="All" />
                    ...
                    <tbody className="divide-y divide-surface-200">
                        {filteredCustomers.map((c, index) => (
                            ...
                            >
                                ...
                                <td className="px-4 py-2 font-medium text-primary-600">{c.code}</td>
                                <td className="px-4 py-2 text-warmgray-900 font-medium">{c.name}</td>
                                <td className="px-4 py-2 text-warmgray-600">{c.phone}</td>
                                <td className="px-4 py-2 text-warmgray-600">{c.email}</td>
                                <td className="px-4 py-2">
                                    <span className="px-2 py-1 rounded-full text-xs font-semibold bg-surface-200 text-warmgray-700">{c.category}</span>
                                </td>
                                <td className="px-4 py-2 text-right font-bold text-warmgray-900">{formatCurrency(c.creditLimit)}</td>
                            </tr>
                        ))}
            ...
            <div className="px-4 py-2 border-t border-surface-200 bg-surface-50 flex items-center justify-between text-xs text-warmgray-500">
                <span>Showing 1 to {filteredCustomers.length} of {filteredCustomers.length} items</span>

...
const dummyDeliveries = [
    {
        id: '1',
        deliveryNumber: 'DO.2024.12.00101',
        orderNumber: 'SO.2024.12.00482',
        customerName: 'APLIKATOR (PKY)',
        deliveryDate: '2024-11-12',
        destination: 'Jl. Raya Utama No. 45',
        status: 'Delivered',
    },
    {
        id: '2',
        deliveryNumber: 'DO.2024.12.00102',
        orderNumber: 'SO.2024.12.00487',
        customerName: 'CV. ZULFA BERKAH',
        deliveryDate: '2024-11-08',
        destination: 'Jl. Sudirman No. 10',
        status: 'In Transit',
    },
];

export default function PengirimanView() {
        ...
        <div className="flex flex-col h-[calc(100vh-6rem)] bg-white rounded-lg shadow-sm border border-surface-200 overflow-hidden">
            ...
            <div className="flex items-center justify-between px-4 py-2 bg-surface-50 border-b border-surface-200">
                <div className="flex items-center gap-2">
                    <Link href="/dashboard/sales/pengiriman/new">
                    ...
                    <tbody className="divide-y divide-surface-200">
                        {filteredDeliveries.map((d, index) => (
                            ...
                            >
                                ...
                                <td className="px-4 py-2 font-medium text-warmgray-900">{d.deliveryNumber}</td>
                                <td className="px-4 py-2 text-primary-600 font-medium">{d.orderNumber}</td>
                                <td className="px-4 py-2 text-warmgray-600 whitespace-nowrap">{formatDate(d.deliveryDate)}</td>
                                <td className="px-4 py-2 text-warmgray-900 font-medium">{d.customerName}</td>
                                <td className="px-4 py-2 text-warmgray-600 truncate max-w-[250px]">{d.destination}</td>
            ...
            <div className="px-4 py-2 border-t border-surface-200 bg-surface-50 flex items-center justify-between text-xs text-warmgray-500">
                <span>Showing 1 to {filteredDeliveries.length} of {filteredDeliveries.length} items</span>
